...
helm pl-plugin --ethAdapter -i $ethValuesPath $ghInfoPath $ethServicePath $qnInfoPath $smartContractInfoPath $ethInfoPath $ethInfoPath $COMPANY_NAME/tmp/ethadapter-values.yaml $COMPANY_NAME/tmp/rpc-address.yaml $COMPANY_NAME/tmp/networkName.yaml -o $COMPANY_NAME/tmp
helm upgrade --install --wait --timeout=600s ethadapter pharmaledger-imi/ethadapter -f $ethServicePath -f $ethInfoPath -f $COMPANY_NAME/tmp/ethadapter-values.yaml -f $COMPANY_NAME/tmp/rpc-address.yaml -f $COMPANY_NAME/tmp/networkName.yaml --set-file config.smartContractInfo=$COMPANY_NAME/tmp/eth-adapter.plugin.json,secrets.orgAccountJson=$COMPANY_NAME/tmp/orgAccount.json
