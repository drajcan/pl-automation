...
variable "ingress_enabled" {
  type        = bool
  description = "Whether to expose Grafana via an ingress to the outside world or not. If 'true' also set valid values for 'ingress_settings' and 'oidc_client_secret'"
  default     = false
}
...
#
# SSO in Grafana with Azure AD Settings
#
...
variable "azuread_auth_settings" {
  type = object({
...
variable "azuread_auth_client_secret" {
  type        = string
